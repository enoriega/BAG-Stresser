...
This script launches multiple user sessions using asyncio and multiprocessing:
- Each session behaves like a loop that runs for the specified duration
- Multiple sessions run concurrently using asyncio (async/await)
- Multiple workers run in separate processes, each executing N sessions

Architecture:
- 1 session, 1 worker: Single session running conversations for duration
- N sessions, 1 worker: N concurrent sessions, each running for duration
- N sessions, M workers: M processes, each running N concurrent sessions
  (Total: N × M sessions executing concurrently)
"""
import os
import sys
import asyncio
...
async def run_multi_session(
    ...
    """
    Run multiple user sessions concurrently using asyncio.

    Each session runs as its own "loop" for the specified duration, and all sessions
    execute concurrently through async/await.
    ...
    Args:
        ...
        stagger_start: If True, add random delay (0-45s) before first session starts
    ...
    # Apply random delay if stagger_start is enabled
    if stagger_start:
        delay = random.uniform(0, 45)
        print(f"{prefix}Staggering start: waiting {delay:.1f} seconds before launching sessions...")
    ...
    async def run_session_wrapper(session_id: int):
        """
        Wrapper to run a session and catch any fatal exceptions.
        Returns None if session fails completely, ensuring other sessions continue.
        """
        try:
            print(f"{prefix}[Session {session_id}] Starting...")
            stats = await simulate_user_session(
                ...
                concurrency=1,  # Each session runs conversations sequentially
                model_filter_file=model_filter_file
            )
            print(f"{prefix}[Session {session_id}] ✓ Completed successfully")
            return stats
        except Exception as e:
            # Catch fatal session-level errors (e.g., missing conversations dir)
            print(f"\n{prefix}[Session {session_id}] ✗ FATAL ERROR: {type(e).__name__}: {str(e)}")
            print(f"{prefix}[Session {session_id}] This session will not produce results, but other sessions continue.")
            return None

    # Create tasks for all sessions - each session manages its own concurrency internally
    tasks = []
    for session_id in range(1, num_sessions + 1):
        print(f"{prefix}[Session {session_id}] Preparing to launch...")
        task = asyncio.create_task(run_session_wrapper(session_id))
        tasks.append(task)
    ...
    # Filter out exceptions and None results, keep successful sessions
    successful_results = []
    failed_count = 0
    for i, result in enumerate(results, 1):
        if isinstance(result, Exception):
            print(f"\n{prefix}[Session {i}] UNEXPECTED FAILURE: {result}")
            failed_count += 1
        elif result is None:
            # Session failed but was handled gracefully
            failed_count += 1
        else:
...
def aggregate_session_stats(all_sessions: list[UserSessionStats]) -> dict:
    ...
    # Calculate message-level error rate
    # Count total messages attempted vs successfully responded to
    ...
    # Failed messages = messages sent but no response received
    ...
    return {
        ...
        'total_user_messages_attempted': total_user_messages_attempted,
        'total_ai_responses_received': total_ai_responses_received,
...
def print_aggregate_report(stats: dict):
    ...
    print(f"  User Messages Attempted: {stats['total_user_messages_attempted']:,}")
    print(f"  AI Responses Received: {stats['total_ai_responses_received']:,}")
    print(f"  Failed Messages: {stats['failed_messages']:,}")
    print(f"  Message Error Rate: {stats['message_error_rate']:.2f}%")
    print()
...
Examples:
  ...
  python multi_session.py --sessions 5 --workers 5 --duration 30
    ...
    parser.add_argument(
        '--stagger-start',
        action='store_true',
        help='Add random delay (0-45s) before first session starts on each worker to avoid connection bursts (default: disabled)'
    )
